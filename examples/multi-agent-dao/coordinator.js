...
async function execute(proposal, result) {
  console.log(`  ↳ EXECUTE: enacting proposal ${proposal.deployHash}, would submit on-chain deploy here (stubbed).`.replace(/\n|\r/g, ''));
}
...
function logTranscript(proposal, result, ctx) {
  ...
  console.log(`    from ${short(proposal.from)}  →  to ${short(proposal.to)}`.replace(/\n|\r/g, ''));
  console.log(`    deploy ${short(proposal.deployHash)}  block ${proposal.blockHeight ?? '?'}  sub_${proposal.subscriptionId ?? '?'}`.replace(/\n|\r/g, ''));
  console.log(`  ${line}`);
