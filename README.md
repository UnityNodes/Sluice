...
| **Hosted MCP (HTTP)** | Zero install, one URL for any client: `https://sluice.unitynodes.com/mcp`. Exposes the 2 read-only feed tools (`recent_deliveries`, `sluice_sandbox_dispatch`) plus all 4 resources, 3 resource templates and 2 prompts; `subscribe_to_events` and `cancel_subscription` stay stdio-only because they sign with your Casper key. Per-client setup in [docs/MCP_CLIENTS.md](docs/MCP_CLIENTS.md). |
