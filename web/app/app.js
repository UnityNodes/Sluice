...
Webhook it to ${wh}, lock ${amt} CSPR."`;
  ...
  async function pubkeyToAccountHash(pubkeyHex) {
    ...
    if (!window.blake2b) await new Promise((r, j) => { const s = document.createElement('script'); s.src = 'https://cdn.jsdelivr.net/npm/blake2b@2.1.4/index.min.js'; s.onload = r; s.onerror = j; document.head.appendChild(s); });
    const out = window.blake2b(32).update(buf).digest();
