...
Across the Casper Agentic Buildathon 2026 submissions, we could not find another that ships this primitive. It is now live on testnet, MIT-licensed.
...
- **MCP server**, one hosted URL for any MCP client, five tools, real transcripts on the landing page.
...
1. **Lowers the bar for "build something agentic on Casper."** An LLM agent integration goes from a multi-week indexer effort to one hosted MCP URL: `https://sluice.unitynodes.com/mcp`.
...
- **v0.2, broader event coverage**: Deploy, Balance, and more derived events. Wallet-signed mutations from the dashboard (contract redesign to use TransactionV1's native `transferred_value`). OpenAPI surface.
