...
- **Verify the honest numbers on screen** match live before each take:
  ...
  - `ACTIVE SUBS` tile = **6 / 7 TOTAL** with its **`1 escrow-backed · 5 demo`** subline (badge + subheader say **`1 active`** — that's the escrow-backed count)
...
- **Everything is browser-only — no terminal, no wallet.** The Sandbox tab has a built-in receiver pre-filled; the live feed's real on-chain swaps arrive on their own (auto every ~15 min).
...
| 5 | 1:30–2:00 | **All in the browser — no terminal.** `/app` → **Sandbox** tab: the **WEBHOOK URL** is pre-filled with a built-in receiver, recipe "any recent transfer", COUNT 3 → click **▶ Fire 3 webhooks**. **DISPATCH RESULTS** instantly show 3 rows — `200 · 100 ms`, `200 · 52 ms`, `200 · …`, each with a signature hash. Then switch to **Live feed** (nav "Live feed") — real on-chain rows **`Contract · Swap @ ffb5a9… → 200`** land on their own (DemoDex swaps, auto every ~15 min). ⚠️ Sandbox = synthetic signed test POSTs (labelled "NO CSPR · no on-chain effect") — do NOT call them on-chain; the on-chain claim belongs only to the feed's `Swap` rows. | "And it all runs from the browser — no CLI, no wallet. I hit Fire, and Sluice sends real, HMAC-signed webhook POSTs straight to your endpoint — there they land, two-hundreds in tens of milliseconds. That's how you harden your receiver before you pay a cent. And in the live feed, these rows are real on-chain DemoDex swaps the matcher caught and delivered, landing on their own." |
