...
2. `ACTIVE SUBS` **6** (or any count that includes demo lanes). Honest value is **1**.
...
| 3 | 0:35–1:05 | Open `/app` (Subscriptions tab). Pan the stat bar — **FUNDS LOCKED 30 CSPR · ACTIVE SUBS 1 · EVENTS DELIVERED 11 · WEBHOOK HEALTH 100%**. Then the table: hover the real lane **`sub_0003`** (30 CSPR, `ecf442…7309`); then hover a **`DEMO`-marked** lane. | "Here's the live dashboard. One real escrow-backed subscription — thirty CSPR locked, funded from a real wallet. The other lanes are clearly marked **DEMO**: they deliver real webhooks so you can watch the feed move, but they hold no escrow, so we never count them as locked funds or as active subscriptions. What you see is what's actually on chain." |
| 4 | 1:05–1:30 | `/app` → **Build** tab. Type into the AI parser: **`watch whales over 100k cspr to dc7252…787c9c`**. The JSON predicate writes itself on the right. Click **Test** against the sample event → **MATCHES**. | "You don't hand-write JSON. Describe the rule in plain English — 'watch whales over 100k CSPR to this address' — and the builder compiles it to a predicate in the browser. Test it against a sample event: it matches." |
| 5 | 1:30–2:00 | Cut to a terminal. Run **`scripts/demo-swap.sh 500000 CSPR USDC`**. Immediately cut back to `/app` (or `/feed`) live feed — a new delivery row lands within a couple of seconds: **`Contract · Swap @ ffb5a9… → 200 · ~85ms`**. | "This isn't a mock. I'll fire a real swap on our DemoDex contract on testnet. The production matcher is watching it — swap transaction, to CSPR.cloud's stream, to a predicate match, to a webhook two-hundred — and there it is in the feed, a real matched event in seconds." |
| 6 | 2:00–2:25 | On `/app`, the **x402** panel: click **"Pull a matched event via x402"**. Show `Settling on-chain…` → settled. Then cut to **cspr.live** showing a `record_delivery` / settlement deploy (`error: None`). | "And the billing is real too. This lane is paid per delivery over x402 — each pull settles a point-one WCSPR micropayment through Casper's hosted facilitator, no wallet needed. Every escrow-backed delivery is a deploy on Casper: your bill is an auditable ledger on cspr.live, not an invoice we made up." |
